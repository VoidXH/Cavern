    ...
    partial class EnhancedAC3Header : IMetadataSupplier {
        ...
        public ReadableMetadata GetMetadata() => new ReadableMetadata(new[] {
            new ReadableMetadataHeader("Bitstream information", new[] {
                ...
                new ReadableMetadataField("frmsiz (in b/f)", $"Bytes per {Blocks * 256} samples", WordsPerSyncframe * 2 + " B"),
                new ReadableMetadataField("frmsiz (in kb/s)", $"Track bitrate", (long)WordsPerSyncframe * SampleRate / (Blocks * 16 * 1000) + " kbps"),
                ...
                new ReadableMetadataField("cmixlev (LtRt)", "Center downmix level in Lt/Rt mode", QMath.GainToDb(GetCenterDownmixLtRt()).ToString("0.0 dB")),
                new ReadableMetadataField("cmixlev (LoRo)", "Center downmix level in Lo/Ro mode", QMath.GainToDb(GetCenterDownmixLoRo()).ToString("0.0 dB")),
