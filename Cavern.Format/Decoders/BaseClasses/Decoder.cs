...
namespace Cavern.Format.Decoders {
    ...
    public abstract class Decoder {
        ...
        /// <summary>
        /// Bit depth of the samples in the stream.
        /// </summary>
        ...
        /// <summary>
        /// Decode a block of big-endian RIFF WAVE data.
        /// </summary>
        internal static unsafe void DecodeBigEndianBlock(BlockBuffer<byte> reader, float[] target, long from, long to, BitDepth bits) {
            ...
            switch (bits) {
                ...
                case BitDepth.Int16:
                    fixed (byte* pSrc = source) {
                        ...
                        while (src != end) {
                            short value = (short)((*src << 8) | src[1]);
                            target[from++] = value * BitConversions.fromInt16;
                            src += sizeof(short);
                        }
                    }
                    break;
                case BitDepth.Int24:
                    fixed (byte* pSrc = source) {
                        ...
                        while (src != end) {
                            int value = (*src << 24) | (src[1] << 16) | (src[2] << 8); // Needs to be shifted into overflow for correct sign
                            target[from++] = value * BitConversions.fromInt32;
                            src += 3;
                        }
                    }
                    break;
        ...
        /// <summary>
        /// Decode a block of little-endian RIFF WAVE data.
        /// </summary>
                    ...
                    fixed (byte* pSrc = source) {
                        ...
                        while (src != end) {
                            target[from++] = ((*(ushort*)src << 8) | *(src += sizeof(ushort)) << 24) *
                                BitConversions.fromInt32; // Needs to be shifted into overflow for correct sign
                            src++;
                        }
                    }
                    break;
